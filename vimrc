...
" :tab options - вивести всі налаштування в новій вкладці
...
set statusline=[B:%n]%1*%r%m%*[%F]\ %2*%S%*\ %=[Column:%2c][Line:%2l/%L][%2p%%]\ [%{&enc}]%y
" Налантування для мобільного
" set statusline=[B:%n]%1*%r%m%*[%f]\ %2*%S%*\ %=[%2c][%2l/%L][%2p%%]\ [%{&enc}]%y
...
" ========== 7. Сторінки вкладок ==========
set showtabline=2       " 0, 1 або 2; коли використовувати рядок сторінок вкладок
...
set tabstop=4	        " Кількість пробілів, якій дорівнює в тексті один <Tab> (стосується буфера)
set shiftwidth=4        " Кількість пробілів у кожному кроці (авто)відступу (стосується буфера)
set smarttab            " <Tab> у відступі вставляє 'shiftwidth' пробілів
set softtabstop=4       " Якщо не нуль, кількість пробілів, які потрібно вставити при <Tab> (стосується буфера)
set shiftround          " Округлити до 'shiftwidth' при «<<» і «>>»
set expandtab           " Замінити <Tab> пробілами в режимі Insert (стосується буфера)
set autoindent          " Автоматично встановити відступ нового рядка (стосується буфера)
...
" Авто видалення пробілів на кінці рядків
